...
struct compressed_string_pool {
  ...
  compressed_string_pool(Range const& strings_seq) {
    ...
    elias_fano::elias_fano_builder positions_builder(positions.back() + 1,
                                                     positions.size());
  ...
  size_t size() const { return m_positions.num_ones() - 1; }

  struct string_enumerator {
    string_enumerator() : m_sp(0) {}
    ...
    char_type next() {
      ...
      if (m_word_begin == m_word_end) {
        ...
        size_t code = 0;
        m_stream_begin +=
            decode_vbyte(m_sp->m_byte_streams, m_stream_begin, code);
   ...
   private:
    string_enumerator(compressed_string_pool const* sp, size_t idx)
        : m_sp(sp), m_word_begin(0), m_word_end(0) {
      std::pair<uint64_t, uint64_t> stream_range =
          m_sp->m_positions.select_range(idx);
  ...
  void map(Visitor& visit) {
    visit(m_dictionary, "m_dictionary")(m_word_positions, "m_word_positions")(
        m_byte_streams, "m_byte_streams")(m_positions, "m_positions");
  }
