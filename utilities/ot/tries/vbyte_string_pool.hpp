...
struct vbyte_string_pool {
  ...
  vbyte_string_pool(Range const& strings_seq) {
    ...
    size_t n = 0, sum = 0;
    for (iterator_t iter = strings_seq.begin(); iter != strings_seq.end();
         ++iter) {
      if (!*iter)
        ++n;
      else
  ...
  struct string_enumerator {
   ...
   private:
    string_enumerator(vbyte_string_pool const* sp, size_t idx) : m_sp(sp) {
      std::pair<uint64_t, uint64_t> string_range =
