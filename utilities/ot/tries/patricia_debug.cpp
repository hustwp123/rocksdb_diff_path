...
struct log_visitor {
  ...
  representation_type leaf(const uint8_t* buf, size_t offset, size_t skip) {
    std::cout << cur_node << "\tleaf\t" << offset << "\t" << skip << "\t"
              << succinct::tries::binary(buf, offset, skip) << std::endl;
    return cur_node++;
  }
  ...
  representation_type node(representation_type& left,
    ...
    std::cout << cur_node << "\tnode\t" << left << "\t" << right << "\t"
              << offset << "\t" << skip << "\t"
              << succinct::tries::binary(buf, offset, skip) << std::endl;
    return cur_node++;
  }
...
int main() {
  ...
  for (size_t i = 0; i < n_strings; ++i) {
    std::cout << strings[i] << "\t"
              << succinct::tries::binary(strings[i].c_str(), 0,
                                         (strings[i].size() + 1) * 8)
              << std::endl;
  }
  ...
  {
    std::cout << std::endl << "Centroid hollow trie:" << std::endl;

    succinct::tries::centroid_hollow_trie ct(
        std::make_pair(strings, strings + n_strings));
