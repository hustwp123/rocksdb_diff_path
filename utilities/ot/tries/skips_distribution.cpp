...
struct skip_counter_visitor {
  skip_counter_visitor() : skip_bits_counts(32), n_strings(0) {}
...
int main(int argc, char** argv) {
  ...
  builder.build(v, succinct::util::lines(strings_file.get(), true),
                succinct::tries::stl_string_adaptor());
  double elapsed =
      double((microsec_clock::universal_time() - m_tick).total_microseconds());

  std::cerr << v.n_strings << " strings processed, "
            << int(elapsed / 1000) / 1000.0 << " seconds elapsed, "
