...
pathdec_tree_stats visit_pathdec(rocksdb::succinct::bp_vector const& bp, size_t node)
{
...
double avg_height(rocksdb::succinct::bp_vector const& bp)
{
...
{
    rocksdb::succinct::util::mmap_lines lines(argv[1]);
    { // compacted trie
	stats_visitor v;
	rocksdb::succinct::tries::compacted_trie_builder<stats_visitor> builder;
	builder.build(v, lines, rocksdb::succinct::tries::stl_string_adaptor());
	...
	rocksdb::succinct::tries::patricia_builder<binary_stats_visitor> builder;
	builder.build(v, lines, rocksdb::succinct::tries::stl_string_adaptor());
    ...
    { // centroid trie
	rocksdb::succinct::tries::path_decomposed_trie<rocksdb::succinct::tries::vbyte_string_pool, false> t(lines);
	std::cout << "centroid_byte_avg_height\t" << avg_height(t.get_bp()) << std::endl;
	std::cout << "centroid_byte_bitsize\t" << rocksdb::succinct::mapper::size_of(t) * 8 << std::endl;
    }

    { // lex trie
	rocksdb::succinct::tries::path_decomposed_trie<rocksdb::succinct::tries::vbyte_string_pool, true> t(lines);
	std::cout << "lex_byte_avg_height\t" << avg_height(t.get_bp()) << std::endl;
	std::cout << "lex_byte_bitsize\t" << rocksdb::succinct::mapper::size_of(t) * 8 << std::endl;
    }

    { //xp, lex trie
      rocksdb::succinct::tries::path_decomposed_trie<rocksdb::succinct::tries::vbyte_string_pool, true> t(lines, false);
      std::cout << "xp lex_byte_avg_height\t" << avg_height(t.get_bp()) << std::endl;
      std::cout << "xp lex_byte_bitsize\t" << rocksdb::succinct::mapper::size_of(t) * 8 << std::endl;
    }

  { // centroid hollow trie
	rocksdb::succinct::tries::centroid_hollow_trie t(lines);
