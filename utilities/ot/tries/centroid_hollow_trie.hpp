...
struct centroid_hollow_trie {
  ...
  size_t index(T const& val, Adaptor adaptor) const {
    ...
    while (true) {
      ...
      forward_enumerator<skips_type> skips_enumerator(m_skips,
                                                      first_child_rank);
      ...
      for (size_t i = 0; i < node_deg; ++i) {
        ...
        if (b != dir) {
          ...
          first_child_rank +=
              (node_deg - child - 1) + (cur_node_pos - child_open) / 2;
      ...
      if (!found_mismatch) {
        ...
        if (node_deg) {
          assert(taken_directions[0] !=
                 0);  // there is at least a right subtrie
          size_t first_right_subtrie = node_end - taken_directions[1] - 1;
          size_t left_leaves =
              (m_bp.find_close(first_right_subtrie) - first_right_subtrie) / 2;
  ...
  void map(Visitor& visit) {
    visit(m_bp, "m_bp")(m_skips, "m_skips");
  }

  bp_vector const& get_bp() const { return m_bp; }

  skips_type const& get_skips() const { return m_skips; }
  ...
  struct centroid_builder_visitor {
    ...
    struct subtree {
      ...
      void append_to(bool close_path, subtree& tree) {
        if (close_path) {
          tree.m_skips.insert(tree.m_skips.end(),
                              m_centroid_path_skips.rbegin(),
                              m_centroid_path_skips.rend());
    ...
    representation_type leaf(const uint8_t* buf, size_t offset,
                             size_t skip) const {
      representation_type ret = boost::make_shared<subtree>();
      return ret;
    }

    representation_type node(representation_type& left,
                             representation_type& right, const uint8_t* buf,
                             size_t offset, size_t skip) const {
      representation_type ret = boost::make_shared<subtree>();
      ...
      ret->m_centroid_path_skips.push_back((uint64_t(skip) << 1) |
                                           uint64_t(centroid_direction));

      size_t closing_centroid_path = left->m_centroid_path_skips.size() +
                                     right->m_centroid_path_skips.size();
      ret->m_bp.reserve(left->m_bp.size() + right->m_bp.size() +
                        closing_centroid_path + 1);
      ret->m_skips.reserve(left->m_skips.size() + right->m_skips.size() +
                           closing_centroid_path);
    ...
    void root(representation_type& root_node) {
      ...
      ret->m_bp.reserve(root_node->m_bp.size() +
                        root_node->m_centroid_path_skips.size() + 2);
      ret->m_skips.reserve(root_node->m_skips.size() +
                           root_node->m_centroid_path_skips.size());
