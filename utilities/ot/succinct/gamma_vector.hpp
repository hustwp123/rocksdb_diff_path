...
struct gamma_vector {
  ...
  gamma_vector(Range const& ints) {
    ...
    for (iterator_t iter = boost::begin(ints); iter != boost::end(ints);
         ++iter) {
  ...
  size_t size() const { return m_high_bits.num_ones() - 1; }
