...
class elias_fano {
 public:
  ...
  void map(Visitor& visit) {
    visit(m_size, "m_size")(m_high_bits, "m_high_bits")(
        m_high_bits_d1, "m_high_bits_d1")(m_high_bits_d0, "m_high_bits_d0")(
        m_low_bits, "m_low_bits")(m_l, "m_l");
  }
  ...
  inline bool operator[](uint64_t pos) const {
    ...
    while (h_pos > 0 && m_high_bits[h_pos - 1]) {
      --rank;
      --h_pos;
  ...
  inline uint64_t select(uint64_t n) const {
    return ((m_high_bits_d1.select(m_high_bits, n) - n) << m_l) |
  ...
  inline uint64_t successor1(uint64_t pos) const { return select(rank(pos)); }
