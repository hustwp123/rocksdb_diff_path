...
class darray {
 public:
  ...
  void map(Visitor& visit) {
    visit(m_positions, "m_positions")(m_block_inventory, "m_block_inventory")(
        m_subblock_inventory, "m_subblock_inventory")(m_overflow_positions,
  ...
  inline uint64_t select(bit_vector const& bv, uint64_t idx) const {
    ...
    if (!reminder) {
      ...
      uint64_t word =
          WordGetter()(data, word_idx) & (uint64_t(-1) << word_shift);
...
struct identity_getter {
  uint64_t operator()(mapper::mappable_vector<uint64_t> const& data,
                      size_t idx) const {
    return data[idx];
  }
};
