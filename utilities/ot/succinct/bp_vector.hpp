...
class bp_vector : public rs_bit_vector {
 public:
  bp_vector() : rs_bit_vector() {}

  template <class Range>
  bp_vector(Range const& from, bool with_select_hints = false,
  ...
  void map(Visitor& visit) {
    ...
    visit(m_internal_nodes, "m_internal_nodes")(
        m_block_excess_min, "m_block_excess_min")(m_superblock_excess_min,
                                                  "m_superblock_excess_min");
  }
 ...
 protected:
  static const size_t bp_block_size =
      4;  // to increase confusion, bp block_size is not necessarily rs_bit_vector block_size
  static const size_t superblock_size = 32;  // number of blocks in superblock

  typedef int16_t
      block_min_excess_t;  // superblock must be at most 2^15 - 1 bits
  ...
  inline bool search_block_in_superblock(uint64_t block, excess_t excess,
