...
struct darray64 {
    ...
    void append1(size_t skip0 = 0) {
      ...
      if (n_ones % subblock_size == 0) {
        subblock_inventory.push_back(
            uint16_t(bits.size() - 1 - block_inventory[n_ones / block_size]));
      }
  ...
  void map(Visitor& visit) {
    visit(m_num_ones, "m_num_ones")(m_bits, "m_bits")(
        m_block_inventory, "m_block_inventory")(m_subblock_inventory,
                                                "m_subblock_inventory");
  }

  size_t num_ones() const { return m_num_ones; }

  bit_vector const& bits() const { return m_bits; }
 ...
 protected:
  static const size_t block_size =
      1024;  // 64 * block_size must fit in an uint16_t (64 is the max sparsity of bits)
