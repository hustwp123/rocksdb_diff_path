...
class freeze_visitor : boost::noncopyable {
 public:
  ...
  template <typename T>
  typename boost::enable_if<boost::is_pod<T>, freeze_visitor&>::type operator()(
  ...
  template <typename T>
  freeze_visitor& operator()(mappable_vector<T>& vec,
...
class map_visitor : boost::noncopyable {
 public:
  ...
  template <typename T>
  typename boost::disable_if<boost::is_pod<T>, map_visitor&>::type operator()(
      T& val, const char* friendly_name) {
    val.map(*this);
    return *this;
  }
  ...
  template <typename T>
  typename boost::enable_if<boost::is_pod<T>, map_visitor&>::type operator()(
      T& val, const char* friendly_name) {
...
class sizeof_visitor : boost::noncopyable {
 public:
  ...
  template <typename T>
  typename boost::enable_if<boost::is_pod<T>, sizeof_visitor&>::type operator()(
      T& val, const char* friendly_name) {
  ...
  template <typename T>
  sizeof_visitor& operator()(mappable_vector<T>& vec,
                             const char* friendly_name) {
    size_t checkpoint = m_size;
    (*this)(vec.m_size, "size");
 ...
 protected:
  ...
  size_node_ptr m_cur_size_node;
    ...
    template <typename T>
size_t freeze(T& val, std::ofstream& fout, uint64_t flags = 0,
...
template <typename T>
size_t freeze(T& val, const char* filename, uint64_t flags = 0,
              const char* friendly_name = "<TOP>") {
...
template <typename T>
size_t map(T& val, const char* base_address, uint64_t flags = 0,
           const char* friendly_name = "<TOP>") {
...
template <typename T>
size_t map(T& val, boost::iostreams::mapped_file_source const& m,
           uint64_t flags = 0, const char* friendly_name = "<TOP>") {
