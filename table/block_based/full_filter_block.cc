...
inline void OtLexPdtFilterBlockBuilder::AddKey(const Slice& key) {
//  fprintf(stderr, "before AddKey() in Otpdtblockbuilder() tyzpdk\n");
  if (filter_bits_builder_ == nullptr) {
    fprintf(stderr, "ERROR! filter_bits_builder is nullptr in OtLexPdtFilterBlockBuilder::AddKey() 20dzk\n");
  }
...
//xp
OtLexPdtFilterBlockReader::OtLexPdtFilterBlockReader(const BlockBasedTable* t,
...
bool OtLexPdtFilterBlockReader::KeyMayMatch(
...
//    fprintf(stderr, "DEBUG 9d533h in OtLexPdtFiltBlockReader::Create()\n");
  ...
  return std::unique_ptr<FilterBlockReader>(
      new OtLexPdtFilterBlockReader(table, std::move(filter_block)));
}

bool OtLexPdtFilterBlockReader::PrefixMayMatch(
...
void OtLexPdtFilterBlockReader::KeysMayMatch(
...
//void OtLexPdtFilterBlockReader::PrefixesMayMatch(
//    MultiGetRange* range, const SliceTransform* prefix_extractor,
//    uint64_t block_offset, const bool no_io,
//    BlockCacheLookupContext* lookup_context) {
//#ifdef NDEBUG
...
//#endif
//  assert(block_offset == kNotValid);
//  MayMatch(range, no_io, prefix_extractor, lookup_context); xxxxx
//}

void OtLexPdtFilterBlockReader::MayMatch(
...
size_t OtLexPdtFilterBlockReader::ApproximateMemoryUsage() const {
  size_t usage = ApproximateFilterBlockMemoryUsage();
#ifdef ROCKSDB_MALLOC_USABLE_SIZE
  usage += malloc_usable_size(const_cast<OtLexPdtFilterBlockReader*>(this));
#else
  usage += sizeof(*this);
...
bool OtLexPdtFilterBlockReader::RangeMayExist(
...
bool OtLexPdtFilterBlockReader::IsFilterCompatible(
